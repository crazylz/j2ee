<template>
  <div>
    ...
    <el-table ref="filterTable" :data="all_tableData.slice(pageIndex*10-10, pageIndex*10)">
      <el-table-column align="center" prop="userId" label="借款人id" sortable>
      ...
      <el-table-column align="center" label="金额" sortable prop="amount">
        <template slot-scope="scope">
          <span>
            ￥
            <span style="color:green;font-size:22px">{{ scope.row.amount }}</span>
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="利率" sortable prop="rate">
        <template slot-scope="scope">
          <span>
            <span style="font-size:22px;color:#ad3618">{{ scope.row.rate }}</span>%
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="分期" sortable prop="installmentNumber">
        <template slot-scope="scope">
          <span style="color:black;font-size:22px">
            ...
            <span style="color:gray;font-size:12px">期</span>
          </span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="每月还款日期" sortable prop="payDayOfMonth">
        <template slot-scope="scope">
          <span style="color:black;font-size:22px">
            {{ scope.row.payDayOfMonth }}
            <span style="color:gray;font-size:12px">号</span>
          </span>
        </template>
      </el-table-column>
